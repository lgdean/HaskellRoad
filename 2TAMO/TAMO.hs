...
contra1 bf = (not (bf True)) && (not (bf False))
-- contra1 bf = not (bf True) || (bf False)

contra2 :: (Bool -> Bool -> Bool)  -> Bool
--contra2 bf = not ( (bf True  True)  
--                || (bf True  False) 
--                || (bf False True) 
--                || (bf False False))
...
-- 2. nope
-- 3. yes
-- 4. yes  (proved on paper: equiv #3, then associativity + commutativity)
--    also equivalent:  (P && Q) ==> R
-- 5. no: f, t, f -> t; f, t, f -> f
--logEquiv3 (\p q r -> (p ==> (q ==> r))) (\p q r -> ((p ==> q) ==> r))
-- 6. yes, but darned if I can prove it without truth tables!
--    intuitively, there's this equivalent statement:  (P || (P && not Q))
-- 7. 
